...
const CANONICAL_STATES = ['Evaluated', 'Applied', 'Responded', 'Interview', 'Offer', 'Rejected', 'Discarded', 'NO APPLY'];
...
function validateStatus(status) {
  ...
  const aliases = {
    ...
    'rechazada': 'Rejected',
    'no aplicar': 'NO APPLY', 'no_aplicar': 'NO APPLY', 'skip': 'NO APPLY', 'monitor': 'NO APPLY',
...
function parseTsvContent(content, filename) {
  ...
  if (content.startsWith('|')) {
    ...
    const col4LooksLikeStatus = /^(evaluated|applied|responded|interview|offer|rejected|discarded|no apply|closed|duplicated|repost|conditional|hold|monitor)/i.test(col4);
    const col5LooksLikeStatus = /^(evaluated|applied|responded|interview|offer|rejected|discarded|no apply|closed|duplicated|repost|conditional|hold|monitor)/i.test(col5);
