...
let reportNum = 54;
for (const [url, company, role, slug] of JOBS) {
  const reportPath = path.join(REPORTS_DIR, `${String(reportNum).padStart(3, '0')}-${slug}-${TODAY}.md`);
  const tsvPath = path.join(BATCH_DIR, `${String(reportNum).padStart(3, '0')}-${slug}.tsv`);
...
`;
  ...
  const tsvLine = `${String(reportNum).padStart(3, '0')}\t${TODAY}\t${company}\t${role}\tEvaluated\t3.5/5\t✅\t[${String(reportNum).padStart(3, '0')}](reports/${String(reportNum).padStart(3, '0')}-${slug}-${TODAY}.md)\t-\n`;
  fs.writeFileSync(tsvPath, tsvLine);

  console.log(`[${String(reportNum).padStart(3, '0')}] ${company} — ${role}`);
  reportNum++;
}

console.log(`\nGenerated ${JOBS.length} reports (054-105)`);
console.log(`Tracker entries ready in: ${BATCH_DIR}`);
