...
# Validate that a URL is a plain http(s) link with no whitespace, control
# chars, or shell/sed metacharacters. Untrusted URLs come from scraped input,
# so reject anything suspicious before it reaches the worker prompt.
validate_url() {
  local u="$1"
  case "$u" in
    http://*|https://*) ;;
  ...
  # Reject whitespace, newlines, and control characters
  if printf '%s' "$u" | LC_ALL=C grep -q '[[:space:][:cntrl:]]'; then
    return 1
  fi
  # Reject shell/sed metacharacters that have no place in a URL
  case "$u" in
    *'`'*|*'$('*|*';'*|*'|'*|*'<'*|*'>'*|*'\'*) return 1 ;;
  esac
  return 0
...
process_offer() {
  ...
  if ! validate_url "$url"; then
    ...
    update_state "$id" "$url" "failed" "$now" "$now" "-" "-" "invalid url (failed validation)" "$(get_retries "$id")"
    echo "    ❌ Skipped #$id: URL failed validation"
    return
  fi
  local retries
  ...
  local prompt
  prompt="Process this job offer. Execute the full pipeline: A-F evaluation + report .md + tracker line. Do NOT generate a PDF."
  ...
  # Use bash parameter expansion (literal replacement) instead of sed. sed
  # treats & and | as special in the replacement, so a normal URL with a query
  # string (?a=1&b=2) would corrupt the substitution, and crafted input could
  # inject sed commands. ${var//find/replace} substitutes the value as data.
  local resolved_prompt="$BATCH_DIR/.resolved-prompt-${id}.md"
  local template
  template="$(cat "$PROMPT_FILE")"
  # Bash 5.2 enables patsub_replacement, which makes & in the replacement expand
  # to the matched text (like sed). Disable it so values are inserted literally.
  shopt -u patsub_replacement 2>/dev/null || true
  template="${template//\{\{URL\}\}/$url}"
  template="${template//\{\{JD_FILE\}\}/$jd_file}"
  template="${template//\{\{REPORT_NUM\}\}/$report_num}"
  template="${template//\{\{DATE\}\}/$date}"
  template="${template//\{\{ID\}\}/$id}"
  printf '%s\n' "$template" > "$resolved_prompt"

  # Launch claude -p worker (uses default model from Claude Max subscription).
  # Explicit tool allowlist instead of --dangerously-skip-permissions: the worker
  # reads CV/JD, fetches the URL, researches comp, and writes the report + tracker
  # line. It has NO Bash, so prompt-injection from a scraped JD cannot run host
  # commands. (No PDF generation, so no generate-pdf.mjs / Bash is needed.)
  local exit_code=0
  claude -p \
