...
Process 10-100+ job URLs in parallel without manual intervention.

Two modes: **conductor --chrome** (navigate portals in real-time) or **standalone** (script for pre-collected URLs).
...
Each worker is a clean `claude -p` child with 200K token context. Conductor just orchestrates.
...
1. **Read state**: `batch/batch-state.tsv` → know what's processed
...
- `--retry-failed` — retry only failed offers
...
- If process dies → re-execute → reads `batch-state.tsv` → skips completed
...
Processed: 47 / 50 URLs
Failed: 2 (login required, dead link)
Skipped: 1 (duplicate)

CSV EXPORT (for your sheet)
─────────────────────────────────────
